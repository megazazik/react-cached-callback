...
export function useGetCallbackInner<C extends ((...args) => (...args) => any)> (
	...
	const isNewValues = !arraysEqual(usedValuesRef.current || [], usedValues || []);
    ...
    return ((...args) => {
        let cache4Function = getCache(callbacksCache.current, FUNCTION_NAME, getKey(...args));
        ...
        if (!cache4Function || isNewValues || !arraysEqual(cache4Function.args, args)) {
            ...
            setCache(callbacksCache.current, FUNCTION_NAME, getKey(...args), cache4Function);
        }
...
export function checkParams (params?: Params | any[], usedParams?: any[]): [Params, any[]] {
