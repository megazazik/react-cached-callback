...
import useGetCallback, { useGetCallbackInner, checkParams } from '../useGetCallback';
...
const mountWithCallbacks = ({ calls, getKey = undefined, usedParams = undefined }) =>
	mount(<TestComponent callsParams={calls} getKey={getKey} usedParams={usedParams} />);
const getCallbacks = (wrapper: ReactWrapper) => wrapper.children().props().callbacks;
...
test('useGetCallback. Params', (t) => {
	...
	const callbacks2 = getCallbacks(mountWithCallbacks({ calls: [['first'], ['second'], []] }));
...
test('useGetCallback. Caching', (t) => {
	const wrapper = mountWithCallbacks({ calls: [['first'], ['second'], ['third']] });
...
test('useGetCallback. checkParams', (t) => {
	...
	t.deepEqual(
		checkParams(undefined, undefined),
		[undefined, undefined],
	);

	t.deepEqual(
		checkParams(1, undefined),
		[1, undefined],
	);

	t.deepEqual(
		checkParams({ index: 0 }, undefined),
		[{ index: 0 }, undefined],
	);

	const getKeyMock = () => '';
	t.deepEqual(
		checkParams(getKeyMock, undefined),
		[getKeyMock, undefined],
	);
...
test('useGetCallback. Using getKey', (t) => {
	...
	t.notEqual(callbacks1[0], callbacks1[1], 'Should not be equals with the same key');
	t.notEqual(callbacks1[1], callbacks1[2], 'Should not be equals with the same key and different params');
	t.equal(callbacks1[2], callbacks1[3], 'Should be equals with the same key and same params');
...
test('useGetCallback. used values', (t) => {
	...
	const callbacks2 = getCallbacks(wrapper);
	t.equal(callbacks1[0], callbacks2[0], 'Callbacks should be equal before values changed 1');
	t.equal(callbacks1[1], callbacks2[1], 'Callbacks should be equal before values changed 2');
	...
	const callbacks3 = getCallbacks(wrapper);
	t.notEqual(callbacks1[0], callbacks3[0], 'Callbacks should not be equal before values changed 1');
	t.notEqual(callbacks1[1], callbacks3[1], 'Callbacks should not be equal before values changed 2');
