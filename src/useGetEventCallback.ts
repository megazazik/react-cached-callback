...
export default function useGetEventCallback<
	C extends (...args) => (...args) => any
>(createCallback: C, params?: Params): C {
	...
	useIsomorphicLayoutEffect(() => {
		...
		Object.keys(callbacksCache.current).forEach(cacheKey => {
			if (callbacksCache.current[cacheKey].next) {
				callbacksCache.current[cacheKey].cached =
					callbacksCache.current[cacheKey].next;
		...
		if (!cache4Function) {
			...
			cache4Function.cached = () => {
				if (
					...
					console.warn(
						'useEventCallbacks. You should not call an event handler while rendering.'
					);
				}
			};
